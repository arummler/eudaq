...
bool operator <(Version u, Version v) {
	return u.version_compare(u.m_version_string, v.m_version_string) == -1;
}
bool operator >(Version u, Version v) {
	return u.version_compare(u.m_version_string, v.m_version_string) == +1;
}
...
bool operator ==(Version u, Version v) {
	return u.version_compare(u.m_version_string, v.m_version_string) == 0;
}
...
auto dummy0 = eudaq::Factory<eudaq::StdEventConverter>::Register<
		YarrRawEvent2StdEventConverter>(
		YarrRawEvent2StdEventConverter::m_id_factory);
}

void YarrRawEvent2StdEventConverter::decodeBORE(
		std::shared_ptr<const eudaq::RawEvent> bore) const {
	...
	std::cout << "producer tag in BORE: " << bore->GetTag("PRODID")
			<< std::endl;
	...
	;
	std::cout << "YarrConverterPlugin: front end type "
			<< m_FrontEndType.at(prodID) << " declared in BORE for producer "
			<< prodID << std::endl;

	// getting the module information for event version >=2.1.0
	jsoncons::json moduleChipInfoJson = jsoncons::json::parse(
			bore->GetTag("MODULECHIPINFO"));
	for (const auto &uid : moduleChipInfoJson["uid"].array_range()) {
		...
		currentlyHandledChip.chipLocationOnModule =
				uid["chipLocationOnModule"].as<unsigned int>();
		currentlyHandledChip.internalModuleIndex =
				uid["internalModuleIndex"].as<unsigned int>();
	...
	for (const auto &uid : m_chip_info_by_uid[prodID]) {
		m_module_size_by_module_index[prodID][uid.internalModuleIndex] =
				uid.moduleType;
		m_plane_id_by_module_index[prodID][uid.internalModuleIndex] = base_id
				+ uid.internalModuleIndex;
		m_module_name_by_module_index[prodID][uid.internalModuleIndex] =
				uid.moduleID;
		std::cout << "Assigning plane ID "
				<< m_plane_id_by_module_index[prodID][uid.internalModuleIndex]
				<< " to module " << uid.internalModuleIndex
				<< " which is called "
				<< m_module_name_by_module_index[prodID][uid.internalModuleIndex]
				<< " and has chip " << uid.name << std::endl;
	}

}

bool YarrRawEvent2StdEventConverter::Converting(eudaq::EventSPC d1,
	...
	if (ev->IsBORE()) {
		if (std::count(m_producerBOREread.begin(), m_producerBOREread.end(),
				prodID)) {
	...
	for (std::size_t currentPlaneIndex = 0;
			currentPlaneIndex < m_plane_id_by_module_index[prodID].size();
			++currentPlaneIndex) {
		...
		unsigned int local_id =
				m_plane_id_by_module_index[prodID][currentPlaneIndex];

		standard_plane_by_module_index[currentPlaneIndex] =
				eudaq::StandardPlane(local_id, "Yarr", local_plane_type);
		// Set the number of pixels
		standard_plane_by_module_index[currentPlaneIndex].SetSizeZS(width,
				height, 0, 32,
	...
	for (auto &block_n : block_n_list) {
		...
		while (it < block.size()) { // Should find as many fragments as trigger per event
			...
			for (unsigned currentHitIndex = 0; currentHitIndex < nHits;
					currentHitIndex++) {
				Fei4Hit hit = *((Fei4Hit*) (&block[it]));
				it += sizeof(Fei4Hit);

				if (hit.col < 1 || hit.col > 400 || hit.row < 1 || hit.row > 384
						|| l1id < 0 || l1id > 32 || tag < 0 || tag > 255) {
					std::cout
							<< "Strange value encountered: and this hit in event "
							<< ev_id << " will not be processed further."
							<< std::endl;
					skipped_hits++;
					std::cout << "col: " << hit.col << " row: " << hit.row
							<< " tot: " << hit.tot << " l1id: " << l1id
							<< " tag: " << tag << " bcid: " << bcid
							<< std::endl;
					continue;
				}
				...
				if ((m_chip_info_by_uid[prodID][block_n].moduleType
						== "ITk_quad")
					...
					standard_plane_by_module_index[m_chip_info_by_uid[prodID][block_n].internalModuleIndex].PushPixelHelper(
						...
						== "ITk_single")
						&& (m_FrontEndType.at(prodID) == "Rd53a")) {
					standard_plane_by_module_index[m_chip_info_by_uid[prodID][block_n].internalModuleIndex].PushPixelHelper(
			...
			if (skipped_hits > 0)
				d2->SetTag("SKIPPEDHITS", skipped_hits);
			fragmentCnt++;
		}
	}
	for (auto it = standard_plane_by_module_index.begin();
			it != standard_plane_by_module_index.end(); ++it) {
